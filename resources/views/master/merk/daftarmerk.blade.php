        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                <table id="style-3" class="table style-3 dt-table-hover">
                    ...
                    <tbody>
                        @foreach ($merks as $datamerk)
                        <tr>
                            <td>{{ $datamerk->id }}</a></td>
                            <td>{{ $datamerk->nama }}</a></td>
                            <td class="text-center">
                                <img class="text-center" src="{{ asset('public/uploads')}}/{{$datamerk->foto_merk}}" style="width: 20%">
                            </td>

                            <td class="text-center">

                                <form method="POST" action="{{ route('merk.destroy', $datamerk->id) }}">
                                    @csrf
                                    @method("DELETE")

                                    <a class="btn btn-light-primary btn-icon bs-tooltip"
                                        href="{{ route('merk.edit', $datamerk->id) }}" data-bs-toggle="tooltip"
                                    ...
                                    <a class="btn btn-light-danger btn-icon bs-tooltip"
                                        href="{{ route('merk.destroy', $datamerk->id) }}" data-bs-toggle="tooltip"
