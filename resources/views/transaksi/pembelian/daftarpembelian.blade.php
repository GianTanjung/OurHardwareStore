        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                <table id="style-3" class="table style-3 dt-table-hover">
                    ...
                    <tbody>
                        @foreach ($listPembelian as $datapembelian)
                        <tr>
                            <td><a href="{{ route('pembelian.show', $datapembelian->id) }}">{{ $datapembelian->kode_nota }}</td>
                            <td><a href="{{ route('pembelian.show', $datapembelian->id) }}">{{ $datapembelian->tgl_pesan }}</td>                         
                            <td>{{ $datapembelian->nama_supplier }}</td>
                            <td>{{ $datapembelian->nama_toko }}</td>
                            <td>{{ $datapembelian->status }}</td>
                            <td class="text-end">@currency($datapembelian->grand_total)</td>

                            {{-- <td class="text-center">

                                <form method="POST" action="{{ route('pembelian.destroy', $datapembelian->id) }}">
                                    @csrf
                                    @method("DELETE")

                                    <a class="btn btn-light-primary btn-icon bs-tooltip"
                                        href="{{ route('pembelian.edit', $datapembelian->id) }}" data-bs-toggle="tooltip"
                                    ...
                                    <a class="btn btn-light-danger btn-icon bs-tooltip"
                                        href="{{ route('pembelian.destroy', $datapembelian->id) }}" data-bs-toggle="tooltip"
