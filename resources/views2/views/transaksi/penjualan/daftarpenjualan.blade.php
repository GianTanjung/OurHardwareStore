        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                <table id="style-3" class="table style-3 dt-table-hover">
                    ...
                    <tbody>
                        @foreach ($listPenjualan as $datapenjualan)
                        <tr>
                            <td><a href="{{ route('penjualan.show', $datapenjualan->id) }}">{{ $datapenjualan->kode_nota }}</td>
                            <td><a href="{{ route('penjualan.show', $datapenjualan->id) }}">{{ $datapenjualan->tanggal_transaksi }}</td>                         
                            <td>{{ $datapenjualan->nama_pelanggan }}</td>
                            <td>{{ $datapenjualan->nama_toko }}</td>
                            <td>{{ $datapenjualan->status }}</td>
                            <td class="text-end">@currency($datapenjualan->grand_total)</td>

                            <td class="text-center">

                                <form method="POST" action="{{ route('penjualan.destroy', $datapenjualan->id) }}">
                                    @csrf
                                    @method("DELETE")

                                    {{-- <a class="btn btn-light-primary btn-icon bs-tooltip"
                                        href="{{ route('penjualan.edit', $datapenjualan->id) }}" data-bs-toggle="tooltip"
                                    ...
                                    <a class="btn btn-light-danger btn-icon bs-tooltip"
                                        href="{{ route('penjualan.destroy', $datapenjualan->id) }}" data-bs-toggle="tooltip"
