                ...
                <table id="style-3" class="table style-3 dt-table-hover">
                    <thead>
                        ...
                        <tr>
                            ...
                            <th class="text-end">Dompet</th>
                            <th class="text-center dt-no-sorting">Action</th>
                        </tr>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($pelanggans as $datapelanggan)
                        <tr>
                            <td><a href="{{ route('pelanggan.show', $datapelanggan->id) }}">{{ $datapelanggan->id }}</a></td>
                            <td><a href="{{ route('pelanggan.show', $datapelanggan->id) }}">{{ $datapelanggan->nama }}</a></td>

                            <td>{{ $datapelanggan->no_hp }}</a></td>
                            <td>{{ $datapelanggan->alamat }}</a></td>
                            <td class="text-center">{{ $datapelanggan->poin }}</a></td>
                            <td class="text-end">@currency($datapelanggan->dompet_virtual)</a></td>

                            <td class="text-center">

                                <form method="POST" action="{{ route('pelanggan.destroy', $datapelanggan->id) }}">
                                    @csrf
                                    @method("DELETE")

                                    {{-- <a class="btn btn-light-primary btn-icon bs-tooltip"
                                        href="{{ route('pelanggan.edit', $datapelanggan->id) }}" data-bs-toggle="tooltip"
                                    ...
                                    <a class="btn btn-light-danger btn-icon bs-tooltip"
                                        href="{{ route('pelanggan.destroy', $datapelanggan->id) }}" data-bs-toggle="tooltip"
